...
public class CPSimilarityRegistrationAnalytic {
	public CPSimilarityTransformation2D apply(Vector<CPPointsPair> fiducials) {
		...
		while (fiducialsE.hasMoreElements()) {
			CPPointsPair pair = (CPPointsPair) fiducialsE.nextElement();
		...
		return (new CPSimilarityTransformation2D(T, R, scale));
	}
	public CPSimilarityTransformation2D applynomessage(Vector<CPPointsPair> fiducials) {
		Enumeration<?> fiducialsE;
