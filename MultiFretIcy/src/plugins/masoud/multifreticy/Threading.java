	   ...
	   public void run(long currentTime,int x) {
		  //thread code
			...
			if (seq2.getLastImage() == null) {MessageDialog.showDialog("seq2 empty"); return;}
				...
				System.out.println("The ratio for " + seq1.getName() + " ÷ " + seq2.getName() + " is: " + K/V + " at time: " + (currentTime-Prestart.startTime) + System.lineSeparator());
	          
				//Add the Ratio to the graph's dataset
				if (!Prestart.offlineBool) {
					KVRatio.addOrUpdate((currentTime-Prestart.startTime)/1000000000,K/V);
				} else { 
